...
_ROLE_COLORS = {
    ...
    "frontdesk": ("#0f766e", "#ecfdf5"),
}

_ROLE_COLORS_DARK = {
    "admin":     ("#f38ba8", "#3d1f2d"),
    "clinician": ("#89b4fa", "#1f2f4f"),
    "frontdesk": ("#5eead4", "#134e4a"),
}
...
def _assignable_roles():
    return ["clinician", "frontdesk", "admin"]
...
class UsersPage(QWidget):
    ...
    def __init__(self):
        ...
        self.frontdesk_chip = QPushButton("Front Desk 0")
        self.frontdesk_chip.setObjectName("usrStatFrontdesk")
        self.frontdesk_chip.setCheckable(True)
        self.frontdesk_chip.clicked.connect(lambda _checked=False: self._set_role_filter("frontdesk"))

        self._role_filter_buttons = {
            "all": self.total_chip,
            "admin": self.admin_chip,
            "clinician": self.specialists_chip,
            "frontdesk": self.frontdesk_chip,
        }
        ...
        self.delete_btn = QPushButton("Delete Disabled")
        self.delete_btn.setObjectName("dangerBtn")
        self.delete_btn.setEnabled(False)
        self.delete_btn.setToolTip("User deletion is disabled. Use Set Active/Inactive instead.")
    ...
    def _set_role_filter(self, role_key: str):
        ...
        if normalized not in {"all", "admin", "clinician", "frontdesk"}:
            normalized = "all"
    ...
    def refresh_users(self):
        ...
        frontdesk_count = sum(1 for user in users if user.get("role") == "frontdesk")
        if hasattr(self, "total_chip"):
            ...
            self.frontdesk_chip.setText(f"Front Desk {frontdesk_count}")
            self._sync_role_filter_buttons()
    ...
    def _count_stale_pending_screenings(self) -> int:
        """Return screenings pending longer than 24 hours."""
        try:
            from auth import get_connection, UserManager as AuthUserManager
            conn = get_connection()
            try:
                # Ensure EMR schema exists before querying.
                AuthUserManager._ensure_emr_schema(conn)
                cur = conn.cursor()
                cur.execute(
                    """
                    SELECT COUNT(*)
                    ...
                    WHERE session_status = 'pending'
                      AND screening_date <= datetime('now', '-24 hours')
                    """
                )
                row = cur.fetchone()
    ...
    def _refresh_integrity_banner(self) -> None:
        stale_count = self._count_stale_pending_screenings()
        if stale_count > 0:
            self.show_notification(
                f"DB Integrity Alert: {stale_count} screening record(s) have been pending for more than 24 hours."
            )
        else:
            self.notify_bar.hide()
    ...
    def delete_user(self):
        QMessageBox.information(
            self,
            "Deletion Disabled",
            "User deletion is disabled. Use Set Active/Inactive to manage access.",
        )
